defmodule ExqUi.RouterPlug do
  ...
  defmodule Router do
    ...
    get "/api/scheduled" do
      ...
      {:ok, json} =
        Jason.encode(%{scheduled: Enum.map(map_score_to_jobs(jobs), &Map.from_struct/1)})
    ...
    get "/api/retries" do
      ...
      {:ok, json} = Jason.encode(%{retries: Enum.map(retries, &Map.from_struct/1)})
    ...
    get "/api/retries/:id" do
      ...
      {:ok, json} = Jason.encode(%{retry: Map.from_struct(retry)})
    ...
    get "/api/processes" do
      ...
      {:ok, json} =
        Jason.encode(%{processes: Enum.map(processes, &Map.from_struct/1), jobs: jobs})
